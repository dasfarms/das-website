(async function(){
  ...
  function isZoneCompatible(p, userZoneNum){
    if(userZoneNum == null) return null; // unknown
    const r = getZoneRange(p);
    if(r.min == null || r.max == null) return null; // unknown
...
const LOW_STOCK = 3;
  ...
  async function loadProducts(){
    // Prefer embedded data so the catalog works when opened locally via file://
  ...
  // returns null for unlimited, else remaining number (inventory - qty already in cart for that size)
  function remainingQty(p, size){
    const so = getSizeObj(p, size);
    if(!so) return 0; // not available = treat as 0
  ...
  function matchesFilters(p, f){
    ...
    // Size filter: must have that size available (and not inventory 0)
    if(f.size && f.size!=='Any'){
      const rem = remainingQty(p, f.size);
      if(rem !== null && rem <= 0) return false;
      // if unlimited (null) it's ok; if rem>0 ok; if size missing it returns 0 and fails
    ...
    // My-zone filter: only show plants compatible with the user's zone
    if(f.myZoneOnly){
      // If user has not set a ZIP/zone, we can't filter reliably
      if(f.userZoneNum == null) return true;
      const compat = isZoneCompatible(p, f.userZoneNum);
      // if plant zone range unknown, keep it visible
  ...
  function bestDefaultSize(p){
    ...
    // otherwise pick first available, even if sold out (so it will show SOLD OUT)
    return sizes[0]?.size || '';
  }
  ...
  function render(products){
    ...
    // enable/disable my-zone filter based on whether user zone is set
    if($('#myZoneOnly')){
      const cb = $('#myZoneOnly');
      cb.disabled = (f.userZoneNum == null);
      if(f.userZoneNum == null) cb.checked = false;

      // show the stored zone in the label
    ...
    grid.querySelectorAll('.catalog-card').forEach(card=>{
      ...
      function refresh(){
        // auto-correct selection if chosen size became sold out
  ...
  try{
    ...
    // explicit Search button (for folks who expect a button)
    if(typeof renderCurrentZoneBadge==='function') renderCurrentZoneBadge();

    const zb = $('#zoneSetBtn');
    if(zb) zb.addEventListener('click', ()=>{ if(window.ZoneWidget && ZoneWidget.open) ZoneWidget.open(); setTimeout(renderCurrentZoneBadge, 150); });
    if(typeof renderCurrentZoneBadge==='function') renderCurrentZoneBadge();

    const searchBtn = $('#searchBtn');
    if(searchBtn) searchBtn.addEventListener('click', ()=>render(products));

    // Set/Change zone button next to the my-zone filter
    const zoneBtn = $('#zoneSetBtn');
    if(zoneBtn){
      zoneBtn.addEventListener('click', (e)=>{
        e.preventDefault();
        try{
          // prefer popup window if available
    ...
    // if zone changes in another window/tab (popup), re-render + update label text
    ...
    window.addEventListener('zonechange', ()=>render(products));
